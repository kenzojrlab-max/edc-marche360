...
import { MOCK_MARCHES, MOCK_PROJETS, formatFCFA, calculateDaysBetween, CONFIG_FONCTIONS } from '../services/mockData';
import { JalonPassationKey, SourceFinancement, StatutGlobal, Marche, Projet } from '../types';

// --- Composant Cellule Document pour le Registre (Action unique : Téléverser) ---
const DocCellInline = ({ doc, label, disabled, onUpload }: { doc?: any, label: string, disabled?: boolean, onUpload?: () => void }) => {
  ...
  return (
    <div className={`flex items-center ml-auto flex-shrink-0 ${disabled ? 'opacity-20 pointer-events-none' : ''}`}>
      <input 
        type="file" 
        ref={fileInputRef} 
...
const MarketList: React.FC<MarketListProps> = ({ mode, readOnly = false }) => {
  ...
  const [formData, setFormData] = useState<Partial<Marche>>({
    ...
    dates_prevues: {
      saisine_cipm: '', examen_dao_cipm: '', ano_bailleur_dao: '', lancement_ao: '', depouillement: '',
      prop_attrib_cipm: '', avis_conforme_ca: '', ano_bailleur_attrib: '', publication: '',
      souscription_projet: '', saisine_cipm_projet: '', examen_projet_cipm: '', ano_bailleur_projet: '',
      signature_marche: '', notification: ''
    } as any,
    statut_global: StatutGlobal.PLANIFIE, hors_ppm: false, exercice: 2024
  });

  const handleDocUpload = (marketId: string, docKey: string, isSpecialDoc?: boolean) => {
    setMarches(prev => prev.map(m => {
      if (m.id === marketId) {
        const mockPiece = { nom: 'Document_PPM.pdf', url: '#', date_upload: new Date().toISOString().split('T')[0] };
        if (isSpecialDoc) {
  ...
  const handleExcelImport = () => {
    ...
    setTimeout(() => {
      ...
      setExcelFile(null);
      const importedMarket: Marche = {
        id: `IMP-${Date.now()}`, objet: 'Marché importé via Excel', fonction_parente: CONFIG_FONCTIONS[0].libelle,
        activite_parente: 'Importation', type_ao: 'AONO', type_prestation: 'Services', montant_prevu: 50000000,
        delai_global_passation: 90, source_financement: currentProjet?.source_financement || SourceFinancement.BUDGET_EDC,
        imputation_budgetaire: 'IMP-001', hors_ppm: false, statut_global: StatutGlobal.PLANIFIE,
        exercice: selectedYear, projet_id: selectedProjetId!, dates_prevues: { saisine_cipm: '2024-10-01' } as any,
        dates_realisees: {} as any, docs: {}, is_infructueux: false, is_annule: false, recours: 'Néant', etat_avancement: 'Inscrit via Excel'
      };
      setMarches([...marches, importedMarket]);
    }, 1500);
  };
                             ...
                             <td colSpan={6 + (jalonCols.length * 2) + 2} className="px-2 py-1.5">
                               <div className="bg-white rounded-lg border border-primary/10 shadow-lg overflow-hidden animate-in slide-in-from-top-1 duration-200">
                                 ...
                                 <div className="p-1.5 grid grid-cols-4 md:grid-cols-6 lg:grid-cols-10 gap-1.5">
                                   ...
                                   <InlineField number="2" label="Intitulé projet (DAO)"><ReadOnlyValue value={m.objet} /><DocCellInline doc={m.docs?.dao} label="DAO" onUpload={() => handleDocUpload(m.id, 'dao')} /></InlineField>
                                   ...
                                   <InlineField number="5" label="Saisine prévisionnelle CIPM"><ReadOnlyValue value={m.dates_realisees.saisine_cipm_prev} isDate /><DocCellInline doc={m.docs?.saisine_prev} label="Saisine Prév" onUpload={() => handleDocUpload(m.id, 'saisine_prev')} /></InlineField>
                                   <InlineField number="6" label="Saisine CIPM* (Transmis.)"><ReadOnlyValue value={m.dates_realisees.saisine_cipm} isDate /><DocCellInline doc={m.docs?.saisine} label="Docs" onUpload={() => handleDocUpload(m.id, 'saisine')} /></InlineField>
                                   <InlineField number="7" label="Examen DAO CIPM*"><ReadOnlyValue value={m.dates_realisees.examen_dao_cipm} isDate /><DocCellInline doc={m.docs?.examen_dao} label="Examen DAO" onUpload={() => handleDocUpload(m.id, 'examen_dao')} /></InlineField>
                                   <InlineField number="8" label="Validation dossier (PV)"><ReadOnlyValue value={m.dates_realisees.validation_dao} isDate /><DocCellInline doc={m.docs?.validation_dao} label="PV" onUpload={() => handleDocUpload(m.id, 'validation_dao')} /></InlineField>
                                   <InlineField number="9" label="ANO Bailleur* (ANO)" disabled={isEDC}><ReadOnlyValue value={m.dates_realisees.ano_bailleur_dao} isDate /><DocCellInline doc={m.docs?.ano_bailleur_dao} label="ANO" disabled={isEDC} onUpload={() => handleDocUpload(m.id, 'ano_bailleur_dao')} /></InlineField>
                                   <InlineField number="10" label="Lancement AO* (Avis)"><ReadOnlyValue value={m.dates_realisees.lancement_ao} isDate /><DocCellInline doc={m.docs?.lancement} label="Avis" onUpload={() => handleDocUpload(m.id, 'lancement')} /></InlineField>
                                   <InlineField number="11" label="Additif (Doc)"><ReadOnlyValue value={m.dates_realisees.additif} isDate /><DocCellInline doc={m.docs?.additif} label="Additif" onUpload={() => handleDocUpload(m.id, 'additif')} /></InlineField>
                                   <InlineField number="12" label="Dépouillement* (PV)"><ReadOnlyValue value={m.dates_realisees.depouillement} isDate /><DocCellInline doc={m.docs?.depouillement} label="PV" onUpload={() => handleDocUpload(m.id, 'depouillement')} /></InlineField>
                                   <InlineField number="13" label="Valid. Évaluation (PV)"><ReadOnlyValue value={m.dates_realisees.validation_eval_offres} isDate /><DocCellInline doc={m.docs?.validation_eval_offres} label="PV" onUpload={() => handleDocUpload(m.id, 'validation_eval_offres')} /></InlineField>
                                   <InlineField number="14" label="ANO bailleurs (ANO)" disabled={isEDC}><ReadOnlyValue value={m.dates_realisees.ano_bailleur_eval} isDate /><DocCellInline doc={m.docs?.ano_bailleur_eval} label="ANO" disabled={isEDC} onUpload={() => handleDocUpload(m.id, 'ano_bailleur_eval')} /></InlineField>
                                   <InlineField number="15" label="Ouvertures Fin. (PV)"><ReadOnlyValue value={m.dates_realisees.ouverture_financiere} isDate /><DocCellInline doc={m.docs?.ouverture_financiere} label="PV" onUpload={() => handleDocUpload(m.id, 'ouverture_financiere')} /></InlineField>
                                   <InlineField number="16" label="Infructueux (Décision)">
                                      ...
                                      <DocCellInline doc={m.doc_infructueux} label="Décision" onUpload={() => handleDocUpload(m.id, 'doc_infructueux', true)} />
                                   </InlineField>
                                   <InlineField number="17" label="Prop. Attribution* (PV)"><ReadOnlyValue value={m.dates_realisees.prop_attrib_cipm} isDate /><DocCellInline doc={m.docs?.prop_attrib_cipm} label="PV" onUpload={() => handleDocUpload(m.id, 'prop_attrib_cipm')} /></InlineField>
                                   <InlineField number="18" label="Avis conforme CA* (Avis)"><ReadOnlyValue value={m.dates_realisees.avis_conforme_ca} isDate /><DocCellInline doc={m.docs?.avis_conforme_ca} label="Avis" onUpload={() => handleDocUpload(m.id, 'avis_conforme_ca')} /></InlineField>
                                   <InlineField number="19" label="ANO Bailleurs* (ANO)" disabled={isEDC}><ReadOnlyValue value={m.dates_realisees.ano_bailleur_attrib} isDate /><DocCellInline doc={m.docs?.ano_bailleur_attrib} label="ANO" disabled={isEDC} onUpload={() => handleDocUpload(m.id, 'ano_bailleur_attrib')} /></InlineField>
                                   <InlineField number="20" label="Publication* (Décis.)"><ReadOnlyValue value={m.dates_realisees.publication} isDate /><DocCellInline doc={m.docs?.publication} label="Décision" onUpload={() => handleDocUpload(m.id, 'publication')} /></InlineField>
                                   <InlineField number="21" label="Notification Attrib. (Notif.)"><ReadOnlyValue value={m.dates_realisees.notification_attrib} isDate /><DocCellInline doc={m.docs?.notification_attrib} label="Notif." onUpload={() => handleDocUpload(m.id, 'notification_attrib')} /></InlineField>
                                   ...
                                   <InlineField number="24" label="Souscription Marché*"><ReadOnlyValue value={m.dates_realisees.souscription_projet} isDate /><DocCellInline doc={m.docs?.souscription} label="Souscription" onUpload={() => handleDocUpload(m.id, 'souscription')} /></InlineField>
                                   <InlineField number="25" label="Saisine Projet* (Trans.)"><ReadOnlyValue value={m.dates_realisees.saisine_cipm_projet} isDate /><DocCellInline doc={m.docs?.saisine_projet} label="Docs" onUpload={() => handleDocUpload(m.id, 'saisine_projet')} /></InlineField>
                                   <InlineField number="26" label="Examen Projet CIPM*"><ReadOnlyValue value={m.dates_realisees.examen_projet_cipm} isDate /><DocCellInline doc={m.docs?.examen_projet} label="Examen Projet" onUpload={() => handleDocUpload(m.id, 'examen_projet')} /></InlineField>
                                   <InlineField number="27" label="Validation (PV)"><ReadOnlyValue value={m.dates_realisees.validation_projet} isDate /><DocCellInline doc={m.docs?.validation_projet} label="PV" onUpload={() => handleDocUpload(m.id, 'validation_projet')} /></InlineField>
                                   <InlineField number="28" label="ANO bailleurs* (ANO)" disabled={isEDC}><ReadOnlyValue value={m.dates_realisees.ano_bailleur_projet} isDate /><DocCellInline doc={m.docs?.ano_bailleur_projet} label="ANO" disabled={isEDC} onUpload={() => handleDocUpload(m.id, 'ano_bailleur_projet')} /></InlineField>
                                   <InlineField number="29" label="Signature Marché (Doc)"><ReadOnlyValue value={m.dates_realisees.signature_marche} isDate /><DocCellInline doc={m.docs?.signature_marche} label="Marché" onUpload={() => handleDocUpload(m.id, 'signature_marche')} /></InlineField>
                                   <InlineField number="30" label="Annulé (Accord CA)">
                                      ...
                                      <DocCellInline doc={m.doc_annulation_ca} label="Accord CA" onUpload={() => handleDocUpload(m.id, 'doc_annulation_ca', true)} />
                                   </InlineField>
                                   <InlineField number="31" label="Notification*"><ReadOnlyValue value={m.dates_realisees.notification} isDate /><DocCellInline doc={m.docs?.notification_cloture} label="Notif" onUpload={() => handleDocUpload(m.id, 'notification_cloture')} /></InlineField>
                                   <InlineField number="32" label="Recours"><ReadOnlyValue value={m.recours} /><DocCellInline doc={m.docs?.recours} label="Recours" onUpload={() => handleDocUpload(m.id, 'recours')} /></InlineField>
                                   <InlineField number="33" label="Etat d'avancement"><span className="text-[7px] font-black text-primary bg-primary/5 px-1 py-0.5 rounded uppercase">{m.etat_avancement}</span><DocCellInline doc={m.docs?.etat_avancement_doc} label="Etat" onUpload={() => handleDocUpload(m.id, 'etat_avancement_doc')} /></InlineField>
                                 </div>
                               </div>
                             </td>
                           </tr>
